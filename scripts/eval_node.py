...
# Q_TABLE_PATH = '/home/user/ros_ws/omnibot/Data/Log_learning/final.csv.npy'
Q_TABLE_PATH = '/home/user/Qtable.csv.npy'
...
CONST_LINEAR_SPEED_FORWARD = 0.2
CONST_ANGULAR_SPEED_FORWARD = 0.0
CONST_LINEAR_SPEED_TURN = 0.1
CONST_ANGULAR_SPEED_TURN = 0.1

def robotSetPos(*args):
    input("press any key...")
...
def createVelMsg(velocity,angle,rotation=0.0):
    ...
    print(f'Sending: {velMsg.linear.x},{velMsg.linear.y},{velMsg.angular.z}')
    return velMsg
    ...
    while rclpy.ok():
        for episode in range(NUM_OF_EPISODES):
            ...
            for step in range(NUM_OF_STEPS_PER_EPISODE):
                ...
                scan_reduction = lidarReduction(lidar, angles)
                ...
                # distance_to_goal = 1000 #getDistanceToGoal(robot_current_x, robot_current_y, X_GOAL, Y_GOAL)
...
def laserCallback(msg: LaserScan):
    ...
    msgScan = msg
    msgScan_ready = True
    # print(lidarScan(msgScan)[1])

def main():
    global node ,velPub
    # global setModelStateClient, velPub
